...
SocketConfig::SocketConfig(int type, const char* ip, u_short port)
{
    ...
    // ******************
    recvBuffer_float = 0.0f;
    recvBuffer_int = 0;
    recvBuffer_flarray = new float[BUFFER_FLARRAY];
    sendBuffer_float = 0.0f;
    sendBuffer_int = 0;
    sendBuffer_flptr = nullptr;
}
// 析构
SocketConfig::~SocketConfig(void)
{
    delete[] socket_ip;
    delete[] recvBuffer_flarray;
}
// 读取
...
DWORD WINAPI socketThreadFunc(LPVOID thParameter)
{
    ...
    bool isWorking = true;
    int mainCommand = 0;
    char* sendSectionPtr = nullptr;  // 分段发送用的指针, 暂时不用
    ...
    if (para->getType() == 1)  // 服务器
    {
        ...
        while (isWorking)
        {
            ...
            case COMMAND_FLOAT:  // 发送float
                ...
                socketSendRet = send(para->clntSockBackend, (const char*)&(para->sendBuffer_float), para->sendDataLen, NULL);
                if (socketSendRet != para->sendDataLen) SocketErrorExit(para);
                // 释放信号量
                winapiStatus = ReleaseSemaphore(para->sockSendSignal, (LONG)1, NULL);
                if (!winapiStatus) SocketErrorExit(para);
                break;
            case COMMAND_INT:  // 发送int
                ...
                if (socketSendRet != sizeof(int)) SocketErrorExit(para);
                // 发送int格式buffer给出的数据
                socketSendRet = send(para->clntSockBackend, (const char*)&(para->sendBuffer_int), para->sendDataLen, NULL);
                if (socketSendRet != para->sendDataLen) SocketErrorExit(para);
                // 释放信号量
                winapiStatus = ReleaseSemaphore(para->sockSendSignal, (LONG)1, NULL);
                if (!winapiStatus) SocketErrorExit(para);
                break;
            case COMMAND_FLARRAY:  // 发送float array
                // 发送数据头
                socketSendRet = send(para->clntSockBackend, (const char*)&mainCommand, sizeof(int), NULL);
                if (socketSendRet != sizeof(int)) SocketErrorExit(para);
                // 发送数据长度
                socketSendRet = send(para->clntSockBackend, (const char*)&(para->sendDataLen), sizeof(int), NULL);
                if (socketSendRet != sizeof(int)) SocketErrorExit(para);
                // 发送float array格式buffer给出的数据
                // 可能比缓冲区大, 采用分段发送?
                socketSendRet = send(para->clntSockBackend, (const char*)para->sendBuffer_flptr, para->sendDataLen, NULL);
                if (socketSendRet != para->sendDataLen) SocketErrorExit(para);
                // 释放信号量
                winapiStatus = ReleaseSemaphore(para->sockSendSignal, (LONG)1, NULL);
                if (!winapiStatus) SocketErrorExit(para);
    ...
    else if (para->getType() == 2)  // 客户端
    {
        ...
        while (isWorking)
        {
            ...
            case COMMAND_FLOAT:  // 发送float
                ...
                if (socketSendRet != sizeof(int)) SocketErrorExit(para);
                // 发送float格式buffer给出的数据
                socketSendRet = send(para->selfSockBackend, (const char*)&(para->sendBuffer_float), para->sendDataLen, NULL);
                if (socketSendRet != para->sendDataLen) SocketErrorExit(para);
                // 释放信号量
                winapiStatus = ReleaseSemaphore(para->sockSendSignal, (LONG)1, NULL);
                if (!winapiStatus) SocketErrorExit(para);
            ...
            case COMMAND_INT:  // 发送int
                ...
                if (socketSendRet != sizeof(int)) SocketErrorExit(para);
                // 发送int格式buffer给出的数据
                socketSendRet = send(para->selfSockBackend, (const char*)&(para->sendBuffer_int), para->sendDataLen, NULL);
                if (socketSendRet != para->sendDataLen) SocketErrorExit(para);
                // 释放信号量
                winapiStatus = ReleaseSemaphore(para->sockSendSignal, (LONG)1, NULL);
                if (!winapiStatus) SocketErrorExit(para);
            ...
            case COMMAND_FLARRAY:  // 发送float array
                ...
                // 发送float array格式buffer给出的数据
                // 可能比缓冲区大, 采用分段发送?
                socketSendRet = send(para->selfSockBackend, (const char*)para->sendBuffer_flptr, para->sendDataLen, NULL);
                if (socketSendRet != para->sendDataLen) SocketErrorExit(para);
                // 释放信号量
                winapiStatus = ReleaseSemaphore(para->sockSendSignal, (LONG)1, NULL);
                if (!winapiStatus) SocketErrorExit(para);
...
DWORD WINAPI recvThreadFunc(LPVOID thParameter)
{
    ...
    bool isWorking = true;
    int recvType = 0;
    int recvSectionDataLen = 0;  // 分段接收长度变量
    char* recvSectionPtr = nullptr;  // 分段接收用的指针
    ...
    if (para->getType() == 1)  // 服务器
    {
        ...
        while (isWorking)
        {
            ...
            case COMMAND_INT:  // 接收int
                ...
                para->sockPauseRecvSignal = SocketResetSemaphore(para, para->sockPauseRecvSignal, 0, 1);
                break;
            case COMMAND_FLARRAY:  // 接收float array
                // 接收数据长度
                socketSendRet = recv(para->clntSockBackend, (char*)&(para->recvDataLen), sizeof(int), NULL);
                if (socketSendRet != sizeof(int)) SocketErrorExit(para);
                // 接收指定长度的数据到float array格式的buffer
                // 可能比缓冲区大, 采用分段接收!
                recvSectionDataLen = 0;
                recvSectionPtr = (char*)para->recvBuffer_flarray;
                while (recvSectionDataLen < para->recvDataLen)
                {
                    socketSendRet = recv(para->clntSockBackend, recvSectionPtr + recvSectionDataLen, para->recvDataLen - recvSectionDataLen, NULL);
                    recvSectionDataLen += socketSendRet;
                }
                // 释放信号量
                winapiStatus = ReleaseSemaphore(para->sockRecvSignal, (LONG)1, NULL);
                if (!winapiStatus) SocketErrorExit(para);
    ...
    else if (para->getType() == 2)  // 客户端
    {
        ...
        while (isWorking)
        {
            ...
            case COMMAND_FLARRAY:  // 接收float array
                ...
                while (recvSectionDataLen < para->recvDataLen)
                {
                    socketSendRet = recv(para->selfSockBackend, recvSectionPtr + recvSectionDataLen, para->recvDataLen - recvSectionDataLen, NULL);
                    recvSectionDataLen += socketSendRet;
                }
                // 释放信号量
                winapiStatus = ReleaseSemaphore(para->sockRecvSignal, (LONG)1, NULL);
...
void SendFloat(SocketConfig* socketconfig, float source)
{
    ...
    // 进行此次发送
    socketconfig->sendDataType = COMMAND_FLOAT;
    // 长度
    socketconfig->sendDataLen = sizeof(float);
    // 信息
    socketconfig->sendBuffer_float = source;
    // 释放信号量, 让Socket线程进行发送
...
void SendInt(SocketConfig* socketconfig, int source)
{
    ...
    WaitAndResetSendSig(socketconfig);
    // 进行此次发送
    socketconfig->sendDataType = COMMAND_INT;
    // 长度
    socketconfig->sendDataLen = sizeof(int);
    // 信息
    socketconfig->sendBuffer_int = source;
    // 释放信号量, 让Socket线程进行发送
    BOOL winapiStatus = ReleaseSemaphore(socketconfig->sockCommandSignal, (LONG)1, NULL);
    if (!winapiStatus) SocketErrorExit(socketconfig);
}
void SendFloatPtr(SocketConfig* socketconfig, const float* source, size_t length)
{
    ...
    WaitAndResetSendSig(socketconfig);
    // 进行此次发送
    socketconfig->sendDataType = COMMAND_FLARRAY;
    // 长度
    socketconfig->sendDataLen = (int)(length * sizeof(float));
    // 信息
    socketconfig->sendBuffer_flptr = source;
    // 释放信号量, 让Socket线程进行发送
    BOOL winapiStatus = ReleaseSemaphore(socketconfig->sockCommandSignal, (LONG)1, NULL);
    if (!winapiStatus) SocketErrorExit(socketconfig);
}
...
float RecvFloat(SocketConfig* socketconfig)
{
    ...
    // 检查数据格式
    if (socketconfig->recvDataType == COMMAND_FLOAT) retValue = socketconfig->recvBuffer_float;
    else SocketErrorExit(socketconfig);
    // 释放信号量, 让主线程接收
...
int RecvInt(SocketConfig* socketconfig)
{
    ...
    WaitAndResetRecvSig(socketconfig);
    // 检查数据格式
    if (socketconfig->recvDataType == COMMAND_INT) retValue = socketconfig->recvBuffer_int;
    else SocketErrorExit(socketconfig);
    // 释放信号量, 让主线程接收
    BOOL winapiStatus = ReleaseSemaphore(socketconfig->sockPauseRecvSignal, (LONG)1, NULL);
    if (!winapiStatus) SocketErrorExit(socketconfig);
...
int RecvFloatPtr(SocketConfig* socketconfig, float* target)
{
    // 复制到指针指定的内存, 同时返回数组大小, 自行保证空间足够
    // 阻塞自身等待数据
    WaitAndResetRecvSig(socketconfig);
    // 检查数据格式
    if (socketconfig->recvDataType == COMMAND_FLARRAY)
    {
        // 拷贝
        memcpy(target, socketconfig->recvBuffer_flarray, socketconfig->recvDataLen);
    }
    else SocketErrorExit(socketconfig);
    // 释放信号量, 让主线程接收
    BOOL winapiStatus = ReleaseSemaphore(socketconfig->sockPauseRecvSignal, (LONG)1, NULL);
    if (!winapiStatus) SocketErrorExit(socketconfig);
    // 返回长度
    return socketconfig->recvDataLen;
}
int RecvFloatPtr(SocketConfig* socketconfig)
{
    // 直接返回数组大小, 自行保证使用时不会接收到数据!
    // 阻塞自身等待数据
    WaitAndResetRecvSig(socketconfig);
    // 检查数据格式
    if (socketconfig->recvDataType != COMMAND_FLARRAY) SocketErrorExit(socketconfig);
    // 释放信号量, 让主线程接收
    BOOL winapiStatus = ReleaseSemaphore(socketconfig->sockPauseRecvSignal, (LONG)1, NULL);
    if (!winapiStatus) SocketErrorExit(socketconfig);
    // 返回长度
...
{
    DWORD threadStatus = WaitForSingleObject(socketconfig->sockSendSignal, INFINITE);
    if (threadStatus) SocketErrorExit(socketconfig);
...
{
    DWORD threadStatus = WaitForSingleObject(socketconfig->sockRecvSignal, INFINITE);
    if (threadStatus) SocketErrorExit(socketconfig);
